...
%interval = 10;%??
nrofpackets = 2;% число копий сообщения
...
for index =1:length( nrofdevices) % цикл по числу устройств   
    ...
    for i = 1:nr % 1?
        ...
        while d>Radius
            ...
            y=-Radius+2*Radius*rand(1,1); %
            d=sqrt(x^2+y^2);
        end
         dist(i)=d;
         % Path_Los=OkumuraHataModel_Typical_Rural(d,1,5,868.1); %8
         Path_Los=46.6777+3*10*log10(d); %9
         P_rx(i)=P_tx-Path_Los; % 10
         P_rx(i)=10^(P_rx(i)/10); % 11
    end
    ...
    p_rslots = zeros(nrofslots, nrofchannels);
        ...
        for p = 1:nrofpackets % цикл по комиям 
            for k = -wk:1:wk % расматриваем 3 канала с нижней частотой с текущей частотой и с верхней 
                ...
                for j = 1:packetduration/timeinterval % двигаемся по времени передачи пакета
                    if (k==0)
                    p_rslots(j+time_offset, freq(i, p))=p_rslots(j+time_offset, freq(i, p))+P_rx(i); %12
                    end
                   %freq(i, 1)
