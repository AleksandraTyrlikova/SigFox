...
// Модель системы. Два устройства соединены каналом с аддитивным гауссовым шумом.

// Задается (из командной стоки или используется параметр по умолчанию.
// lossDb - затухание в dB.
// txPowerW - мощность передачи в W.
// phyRate - скорость передачи bit/sec.
// pktSize - длинна пакета.
// simDuration - время моделирования.

// Моделирование
// Уровень шума рассчитывается с учетом того, что температура приемного устройства 290 градусов по кельвину (16.85 по Цельсию)
// Передается столько пакетов сколько поместится во время моделирования.

//Результаты моделирования.
// Выдается информация о каждом переданном пакете.
// throughput - число переданных бит/ время моделирования.
// phy rate - скорость передачи на физическом уровне.
// shannon capacity - пропускная способность канала рассчитанная по формуле Шеннона.
// Полоса канала????
int main (int argc, char** argv)
{
  ...
  std::string channelType ("ns3::SingleModelSpectrumChannel");// заменить при использовании других моделей канала.
  ...
  channelHelper.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");// заменить при использовании другой модели затухания.
